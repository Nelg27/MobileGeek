...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills red nav-stacked'>
			<li class='active bg-black'><a href='#'><h4>Marque</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
				...
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>".CURRENCY." $pro_price.00
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>Acheter</button>
								</div>
							</div>
						</div>	
			";
...
if (isset($_POST["Common"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "SELECT p.product_id,p.product_title,p.product_price,p.product_image,c.id,c.qty FROM products p,cart c WHERE p.product_id=c.p_id AND c.ip_add='$ip_add' AND c.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			?>
				<a style="float:right;" href="cart.php" class="btn btn-warning">Voir le panier&nbsp;&nbsp;<span class="glyphicon glyphicon-edit"></span></a>
			<?php
			exit();
		}
	}
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div >
						...
						<b>Produit est supprimé du panier</b>
				</div>";
		exit();
	}
}
